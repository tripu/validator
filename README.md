# The Nu Html Checker (v.Nu) ![Chat room](https://goo.gl/1kHqwI) ![Download
latest](https://goo.gl/3PC2Qn)

The Nu Html Checker (v.Nu) is the backend of
[checker.html5.org](https://checker.html5.org/),
[html5.validator.nu](https://html5.validator.nu), and
[validator.w3.org/nu](https://validator.w3.org/nu/). Its [source code is
available](https://github.com/validator/validator), as are [instructions on
how to build, test, and run the code](https://validator.github.io/validator
/#build-instructions). It is released as two packages:

  * `vnu.jar` is a portable standalone version for [batch-checking documents
  from the command line](https://validator.github.io/validator/#usage) and
  from other scripts/apps, or for [deploying the checker as a self-contained
  service](https://validator.github.io/validator/#standalone) * `vnu.war`
  is for [deploying the checker service through a servlet container such
  as Tomcat](https://validator.github.io/validator/#servlet)

**Note:** The vnu.jar and vnu.war packages require a Java 8 environment;
they won’t run in Java 7 or older environment.

You can [get the latest
release](https://github.com/validator/validator/releases/latest) or run
`[npm install vnu-jar`](https://www.npmjs.com/package/vnu-jar), `[pip
install html5validator`](https://github.com/svenkreiss/html5validator)
or `[brew install vnu`](https://libraries.io/homebrew/vnu), and see the
**Usage** and **Web-based checking** sections below. Or automate your HTML
checking with a frontend such as:

  * [Grunt plugin for HTML
  validation](https://github.com/jzaefferer/grunt-html) or [Gulp plugin for
  HTML validation](https://github.com/watilde/gulp-html) * [html5validator
  `pip` package](https://github.com/svenkreiss/html5validator)
  (for HTML checking integration in Travis CI, CircleCI, CodeShip,
  [Jekyll](https://jekyllrb.com/), [Pelican](https://blog.getpelican.com/)
  etc.)

  * [LMVTFY: Let Me Validate That For You](https://github.com/cvrebert/lmvtfy/)
  (auto-check HTML of JSFiddle/JSBin etc. links in github issue comments)

## Usage

Use the `vnu.jar` HTML checker as an executable for command-line checking
of documents by invoking it like this:
      ...
      java -jar ~/vnu.jar [--errors-only] [--Werrors] [--exit-zero-always]
           ...
           [--skip-non-html] [--no-langdetect] [--help] [--verbose]
           [--version] FILES

**Note:** In these instructions, replace "~/vnu.jar" with the actual path
...
#### \--Werrors
    ...
    Makes the checker exit non-zero if any warnings are encountered (even
...
#### \--filterfile FILENAME
    ...
    expression or starts with "#" to indicate the line is a comment. Any
    error message or warning message that matches a regular expression in
    the file is filtered out (dropped/suppressed).
...
#### \--format format
    ...
    possible values: "gnu", "xml", "json", "text" [see information at
    URL below]
...
#### \--skip-non-html
    ...
    Skip documents that don’t have *.html, *.htm, *.xhtml, or *.xht
    extensions.
...
#### \--no-langdetect
    ...
    Disables language detection, so that documents are not checked for
    missing or mislabeled html[lang] attributes.
...
#### \--verbose
    ...
    Specifies "verbose" output. (Currently this just means that the names
    of files being checked are written to stdout.)
...
client](https://validator.github.io/validator/#usage)ーcan be run as an HTTP
service, similar to [checker.html5.org](https://checker.html5.org/),
[html5.validator.nu](https://html5.validator.nu/), and
[validator.w3.org/nu](https://validator.w3.org/nu/), for browser-based checking
of HTML documents over the Web. To that end, the checker is released as two
separate packages:
...
Both deployments expose a REST API that enables checking of HTML documents
from other clients, not just web browsers. And the `vnu.jar` package also
includes a simple HTTP client that enables you to either send documents to a
locally-running instance of the checker HTTP serviceーfor fast command-line
checkingーor to any remote instance of the checker HTTP service running
anywhere on the Web.

The [latest releases of the vnu.jar and vnu.war
packages](https://github.com/validator/validator/releases/latest) are available
from the `validator` project at github. The following are detailed instructions
on using them.

**Note:** Replace "~/vnu.jar" or "~/vnu.war" below with the actual paths to
...
You’ll see a form similar to
[validator.w3.org/nu](https://validator.w3.org/nu/) that allows you to enter
the URL of an HTML document and have the results for that document displayed
in the browser.
...
To run the checker inside of an existing servlet container such as Apache
Tomcat you will need to deploy the `vnu.war` file to that server following
its documentation. For example, on Apache Tomcat you could do this using
the [Manager](https://tomcat.apache.org/tomcat-8.0-doc/manager-howto.html)
application or simply by copying the file to the `webapps` directory (since
that is the default `appBase` setting). Typically you would see a message
similar to the following in the `catalina.out` log file.
...
**Note:** You may want to customize the `/WEB-INF/web.xml` file inside the
WAR file (you can use any ZIP-handling program) to modify the servlet filter
...
You can also use `vnu.jar` from the command line to either send documents to
a locally-running instance of the checker HTTP serviceーfor fast command-line
...
as [html5.validator.nu/](https://html5.validator.nu/), use the
nu.validator.client.host and nu.validator.client.port options, like this:
...
When using `vnu.jar` for sending documents to an instance of the checker
HTTP service for checking, you can set Java system properties to control
...
ones by setting the value of the `nu.validator.client.level` system property
to `error`, like this:
...
parameters](https://github.com/validator/validator/wiki/Service-%C2%BB-Common-
params).
...
#### nu.validator.client.level
    ...
    warning-level messages, and only show error-level ones, set this property
    to "error".
...
Follow the steps below to build, test, and run the checker such that you
can open `http://localhost:8888/` in a Web browser to use the checker Web UI.
...
Use `python ./build/build.py --help` to see command-line options for
controlling the behavior of the script, as well as build-target names you
can call separately; e.g.:
